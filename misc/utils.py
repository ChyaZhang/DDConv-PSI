...
def read_pred_and_gt(pred_file,gt_file):
    ...
    with open(gt_file) as f:
        for line in f.readlines():
            ...
            points_r = []
            if num>0:
                points_r = np.array(line_data[2:]).reshape(((len(line)-2)//3,3))
                gt_data[idx] = {'num': num, 'points':points_r[:,0:2], 'sigma': points_r[:,2]}
            else:                
                gt_data[idx] = {'num': 0, 'points':[], 'sigma':[]}

    return pred_data, gt_data

def adjust_learning_rate(optimizer, base_lr1, max_iters,
        cur_iters, power=0.9):
