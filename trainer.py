...
class Trainer():
    def __init__(self, cfg_data, pwd):
        ...
        if cfg.OPT == 'Adam':
            self.optimizer = optim.AdamW([{'params':self.net.Extractor.parameters(), 'lr':cfg.LR_BASE_NET, 'weight_decay':1e-5}])
    ...
    def train(self): # training for all datasets
        ...
        for i, data in enumerate(self.train_loader, 0):
            ...
            pre_map, binar_map = self.net(img,gt_map)
            ...
            lr1 = adjust_learning_rate(self.optimizer,
                            cfg.LR_BASE_NET,
                            self.num_iters,
                            self.i_tb)
            ...
            if (i + 1) % cfg.PRINT_FREQ == 0:
                ...
                print( '[ep %d][it %d][loss %.4f][lr1 %.4f][%.2fs]' % \
                        (self.epoch + 1, i + 1, head_map_loss.item(), self.optimizer.param_groups[0]['lr']*10000, self.timer['iter time'].diff) )
            if  i %100==0:
                ...
                vis_results('tmp_vis', 0, self.writer, self.restore_transform, img, pre_map[0].detach().cpu().numpy(), \
                                 gt_map[0].detach().cpu().numpy(),binar_map.detach().cpu().numpy(),boxes)    
    def get_boxInfo_from_Binar_map(self, Binar_numpy, min_area=10):
            ...
            with torch.no_grad():
                ...
                if h*w< slice_h*2*slice_w*2 and h%16 == 0 and w %16 == 0:
                    ...
                    for i in range(0, nz, period):
                        for i in self.net(crop_imgs[i:min(nz, i+period)],mask_gt = None, mode='val'):
                            crop_pred = i.cpu()
                ...
                binar_map = torch.where(pred_map >= 0.5, a, b)
                ...
                tp_s, fp_s, fn_s, tp_l, fp_l, fn_l = eval_metrics(pred_data,gt_data)
